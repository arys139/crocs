...
$cleanfilename = preg_replace(array('/\s{2,}/', '/[\t\n]/'), ' ', $cleanfilename);
 ...
 //Search database
 $sql = "SELECT CROCS_FILE_UPLOADED, CROCS_FILE_NAME, dbms_lob.getlength(CROCS_FILE_UPLOADED) AS COLUMNSIZE ";
 $sql = $sql."FROM DATA_MAPPING_CROCS_ATTACHMENT WHERE CROCS_ATTACHMENT_TYPE = 'HA' AND CROCS_FILE_NAME = '".$cleanfilename."' AND CROCS_LV_NUMBER = '".$CE_HOSTNAME."'";
 ...
 while ($row = oci_fetch_assoc($stid) ) {
    ...
    $CROCS_FILE_UPLOADED = $row['CROCS_FILE_UPLOADED']->load();
